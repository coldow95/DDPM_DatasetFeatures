...
def start_clipFID(source, test, batch_size=16):

    device = "cuda" if torch.cuda.is_available() else "cpu"
    model, preprocess = clip.load("ViT-B/32", device=device)
    fname = "results_fcd.txt"

    tensor_source = from_imgs_folder_to_process_tensor(source, preprocess, sourceFlag = True)
    tensor_test = from_imgs_folder_to_process_tensor(test, preprocess)
